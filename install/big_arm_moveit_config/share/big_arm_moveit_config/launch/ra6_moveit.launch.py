...
#         控制節點段與 big_arm_bringup/launch/ra6_control.launch.py 相同 [SYNC]
...
# [本包] 使用本套件檔案（big_arm_moveit_config/config/）:
...
# =====================================================================
# [overlay] big 臂單臂 MoveIt launch — ra6_moveit.launch.py
#   (本包 big_arm_moveit_config: MoveIt 設定; 完全自足, 不依賴控制包)
#
...
#
#   與官方的差異:
#     [NEW]    全節點進 namespace (/big_arm), controller_manager/robot_description 相對名
#              (控制節點與本套件 ra6_control.launch.py 相同; 改控制層時與 big_arm_bringup/launch/ra6_control.launch.py 同步 [SYNC])
#     [REVISE] SRDF / joint_limits 用本套件前綴版 (官方靜態檔無前綴, 對不上 big_joint_*)
...
#     [NEW]    參數有預設值 (ra610_1476 / ecat / big_ / big_arm), 一鍵啟動:
...
THIS_PKG    = "big_arm_moveit_config"   # 本包 (SRDF/joint_limits/moveit_controllers)
DEFAULT_RA_TYPE = "ra610_1476"        # revise: small 版改 ra605_710
DEFAULT_TF_PREFIX = "big_"            # revise: small 版改 small_
DEFAULT_NAMESPACE = "big_arm"         # revise: small 版改 small_arm
...
def launch_setup():
    ...
    # [REVISE] SRDF: 本套件前綴版 (big_joint_* / big_link_*)
    ...
    # [REVISE] joint_limits: 本套件前綴版 (big_joint_*, 速度值沿用 hiwin)
    ...
    # =================================================================
    # 控制層節點 — 與 big_arm_bringup/launch/ra6_control.launch.py 相同 [SYNC]
    # =================================================================
    ...
    control_node = Node(
        ...
        remappings=[
            # [REVISE] 相對名, namespace 下對到 /big_arm/robot_description
    ...
    move_group_node = Node(
        ...
        namespace=namespace,                 # [NEW] -> /big_arm/move_group
        output="screen",
        parameters=[
            robot_description,
            robot_description_semantic,
    ...
    #   本包複本已注入 Move Group Namespace: /big_arm)
    ...
    rviz_node = Node(
        ...
        namespace=namespace,                 # [NEW] MotionPlanning panel 對上 /big_arm/move_group
        output="log",
...
def generate_launch_description():
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            "tf_prefix",
            default_value=DEFAULT_TF_PREFIX,  # [NEW] 預設 big_
            description=(
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            "namespace",
            default_value=DEFAULT_NAMESPACE,  # [NEW] 預設 big_arm
