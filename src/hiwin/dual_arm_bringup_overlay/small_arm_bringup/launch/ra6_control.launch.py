...
THIS_PKG = "small_arm_bringup"

# [REVISE] 本臂專屬預設值（與 small_arm_moveit_config 的 ra6_moveit.launch.py 一致）。
#   原因: 本套件 ra6_controllers.yaml 已寫死 small_joint_*，若忘帶 tf_prefix 會造成
#   URDF(無前綴) 與 controllers(small_) 關節名不一致 -> 控制器認領介面失敗卡 inactive。
