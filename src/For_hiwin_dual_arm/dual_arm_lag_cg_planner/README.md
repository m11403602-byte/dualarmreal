...
使用 C++17 + Eigen3 實作。內層採用 **純 Lagrangian（決策變數 V=[X;λ;S]）+ CG-FR（Fletcher-Reeves 共軛梯度）+ 1D Newton 線搜索**。
...
內層 (cg_solver):         純 Lagrangian 單迴圈 (V=[X;λ;S] ∈ ℝ^1116) + CG-FR + 線搜索
```
...
所有原始碼（核心演算法 + MoveIt 插件介面）編進**單一共享庫** `libdual_arm_lag_cg_planner.so`。

| 組成 | 角色 |
|------|------|
| `cg_solver` | 第 1 層: 內層 CG 求解器（V=[X;λ;S]）+ FK + 包覆球 + 危險因子 |
...
### 內層：純 Lagrangian 共軛梯度求解器 (`CgSolver::run_lag`)
...
- **`config/`**：**執行期參數層**——存放 move_group 啟動時讀入的 yaml 參數，需複製為 `hiwin_dual_arm/config/dual_arm_lag_cg_planning.yaml`。
- **`include/dual_arm_lag_cg_planner/`**：**介面宣告層**。
- **`src/`**：**演算法實作層**。
...
| `dual_arm_lag_cg_planner.xml` | pluginlib 插件描述（讓 MoveIt2 動態載入） |
| `README.md` | 本文件 |
| `PARAMETERS.md` | 參數對照表（可調參數） |
| `config/dual_arm_lag_cg_planner.yaml` | 規劃器參數（需複製為 `hiwin_dual_arm/config/dual_arm_lag_cg_planning.yaml`） |
| `cg_solver.hpp/.cpp` | 第 1 層：FK、包覆球、危險因子、純 Lagrangian CG 求解 |
...
1. 把本包 `config/dual_arm_lag_cg_planner.yaml` 複製為
   `hiwin_dual_arm/config/dual_arm_lag_cg_planning.yaml`（檔名依規劃器而不同）。
