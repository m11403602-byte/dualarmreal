# 參數對照表 — dual_arm_lag_gd_planner

本規劃器所有可調參數皆在 `config/dual_arm_lag_gd_planning.yaml`（本表 = yaml 註解 + 補充說明）。
...
| 參數 | 預設 | 說明 |
|------|------|------|
| `export_csv_prefix` | `"./lag_data"` | 匯出根目錄：`<prefix>/<unix秒>_GD/`（每次匯出自成時間戳資料夾，不互相覆蓋；export_level=0 時不匯出）|
