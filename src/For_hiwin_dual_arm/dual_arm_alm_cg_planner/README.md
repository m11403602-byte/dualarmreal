...
所有原始碼（核心演算法 + MoveIt 插件介面）編進**單一共享庫** `libdual_arm_alm_cg_planner.so`。
...
- **`include/dual_arm_alm_cg_planner/`**：**介面宣告層**。
- **`src/`**：**演算法實作層**。
...
| `dual_arm_alm_cg_planner.xml` | pluginlib 插件描述（讓 MoveIt2 動態載入） |
| `README.md` | 本文件 |
| `PARAMETERS.md` | 參數對照表（可調參數） |
| `config/dual_arm_alm_cg_planning.yaml` | 規劃器參數（需複製到 `hiwin_dual_arm/config/`） |
...
1. 把本包 `config/dual_arm_alm_cg_planning.yaml` 複製到 `hiwin_dual_arm/config/`（檔名依規劃器而不同）。
